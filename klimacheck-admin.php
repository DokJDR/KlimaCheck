...
function klimacheck_shortcode_render( $atts ) {
    ...
    foreach ( $candidates as $cid => $c ) {
        ...
        $cand = array(
            ...
            'full_statement_url' => '', // placeholder – can be extended
            'responses' => array(),
        );
        for ( $q = 1; $q <= 10; $q++ ) {
    ...
    (function(){
        ...
        var candidates = DATA.candidates;  // array of {name, party, photo_url, full_statement_url, responses: {1:{answer,text},...}}

        var userAnswers = {};   // {questionNumber: 'yes'|'partially'|'no'}
        var userWeights = {};   // {questionNumber: true/false}
        var currentQ    = -1;   // -1 = intro, 0..9 = questions, 10 = results
        ...
        function renderResults() {
            ...
            for (var i = 0; i < results.length; i++) {
                        ...
                        '<div class="kc-detail-q">' +
                            ...
                            '<div class="kc-detail-q-body">' +
                                ...
                                '<div>' +
                                    ...
                                    '<span class="' + badgeClass(userAns) + ' kc-user-badge" style="opacity:.7" title="Ihre Antwort">\uD83D\uDC64 ' + esc(answerLabel(userAns)) + '</span>' +
                                '</div>' +
                ...
                if (c.full_statement_url) {
                    html += '<a class="kc-detail-link" href="' + esc(c.full_statement_url) + '" target="_blank" rel="noopener">Ausf\u00fchrliche Stellungnahme ansehen &rarr;</a>';
                }
